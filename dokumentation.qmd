...
population_per_train <- ggplot(data = german_states_weighted) +
  ...
  heatmap_theme +
  scale_fill_gradient(trans = "log", labels = c(0.0001, 0.00005), breaks = c(0.0001, 0.00005)) 
...
plot_tripple_comparison <- function(population_density, train_density, population_per_train) {
  plot_grid(
    ...
    nrow = 1, 
    labels = c("Bevölkerungsdichte(1/km²)", "Bahnhofdichte (1/km²)", "Bahnhöfe pro Person"), 
    hjust = 0,
    vjust = 1)  
}
...
$$
(1: n/population 2: population/area 3: n/area als Verhältnis von 1 und 2).
Es ist ein Trennlinie zwischen den alten und neuen Bundesländern zu erahnen.
...
# todo dataquality
# german_plz_weighted %>% arrange((einwohner/qkm))
...
stations_per_population.plz <- ggplot(data = german_plz_weighted) +
  geom_sf(
    aes(fill = n/einwohner),
    color = NA
  ) +
  scale_fill_gradient(trans = "log") +
  heatmap_theme

plot_tripple_comparison(population_density.plz, station_density.plz, stations_per_population.plz)
```

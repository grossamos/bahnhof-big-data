...
In allen drei Karten stechen die Stadtstaaten herraus. Auch kann man die dritte Karte als Verhältnissdiagramm zu den ersten Beiden ansehen (1: n/population 2: population/area 3: n/area als Verhältniss von 1 und 2). So könnte man aus den ersten Karten geographische Zusammenhänge erahnen, welche durch die dritte Karte revidiert werden. Hierdurch wird die initiale Vermutung bestätigt, dass es Unterschiede zwischen Stadt und Land gibt aber es keiwne klaren unterschiede im Bundesländervergleich gibt. Dies heißt trotzdem nicht, dass es keine Unterschiede gibt und alle Bundesländer gleich aufgestellt sind.
...
Zunächst müssen die Daten wieder angepasst werden. Hierzu werden die Postleitzahlen nach den ersten beiden Ziffern gruppiert und die Summe aller Einwohner sowie aller Bahnhöfe in diesem Gebiet hinzugefügt. Darauf wird mit *st_area* die Fläche jedes Gebietes aus den Geometriedaten berechnet, welches sehr rechenintensiv ist. Im Amschluss werden wieder drei Karten zur Befölkerungsdichte, Bahnhofsdichte und Anzahl der Bahnhöfe pro Person erstellt.

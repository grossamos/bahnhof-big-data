...
Für die Fragestellung (siehe unten) mussten mehrere Datenquellen gesucht und kombiniert werden.

#### Bevölkerungsdaten
[https://www.suche-postleitzahl.org/downloads](https://www.suche-postleitzahl.org/downloads)

1. Liste aller Postleitzahlen in Deutschland mit 
   - Einwohnerzahl
   - Fläche
   - Position

2. Gebietsgrenzen mit Polygondaten aller Postleitzahlen


#### Stationsdaten
[https://data.deutschebahn.com/dataset/data-stationsdaten.html](https://data.deutschebahn.com/dataset/data-stationsdaten.html)

1. Liste aller deutschen Personenbahnhöfe
   - Bahnhofskategorie
   - Adresse
   - Aufgabenträger
   - Verkehrsverbund
...
Da (todo vormulierung) wir uns später über die Lücken im Bahnhofsnetz gewundert haben, wurde die vollständigkeit [hier](https://www.deutschebahn.com/pr-hamburg-de/DB-im-Norden-1/Regionale-Themen/Bahnhoefe-der-Deutschen-Bahn-6121238) nocheinmal überprüft. (Anzahl ca.5400)

#### Shapefiles
[https://geodata.lib.utexas.edu/catalog/stanford-nh891yz3147](https://geodata.lib.utexas.edu/catalog/stanford-nh891yz3147)

Polygondaten aller Bundesländer zur Kartenvisualisierung.
...
#### Top 10 Städte nach Bahnhofanzahl
todo: 
* top 10 städte nach anzahl bahnhöfe (barchart) (zusätzlich aufteilung in kategorien)

```{r}
t = (table(bahnhof$Ort))
t.order = as.data.frame(t[order(t, decreasing = T)])

t.order
t.order[1:10,]$Var1
filter(bahnhof, Ort %in% t.order[1:10,]$Var1)

str(bahnhof)

top10 = filter(bahnhof, Ort %in% t.order[1:10,]$Var1)
top10.order = ordered(top10, levels = t.order[1:10,]$Var1)
top10.order
...
ggplot(data=top10) +
  ...
  labs(
    y = "Anzahl Bahnhöfe",
    title = "Top 10 Städte nach Bahnhöfen",
...
#### eventuel?: gleiches barchart aber mit bundesländern

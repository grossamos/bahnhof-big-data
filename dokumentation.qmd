...
Insgesammt zehn Zeilen enthalten `NA`s. Diese können aber mit den Stationswebseiten der Deutschen Bahn[^4] ergänzt werden. Einzelne Postleitzahlen (überwiegend in Sachsen) mangeln eine führende "0". Diese wird ebenfalls ergänzt.
...
Für die Kartenanalysen dieser Arbeit werden Polygonaldaten in Form von Shapefiles verwendet. Miteinbezogen wird ein Datensatz mit Bundesländern [^6] und einer mit allen Postleitzahlen [^7]. Der Postleitzahldatensatz enthält Populations- und Flächendaten (`einwohner`, `qkm`). Der Bundesländerdatensatz wird manuel um diese Daten ergänzt [^8].
...
Dabei wird zuerst die genauen Lageverteilung der Stationen betrachtet und anschließend in verschiedenen Granularitätstufen mit Populationsdaten verglichen.
...
Anschließend werden falsche Koordinaten und Stationen, die nicht gefunden werden konnten herrausgefiltert. Die bearbeiteten Daten beinhaltet letztendlich `r nrow(stations_geo_loc) / previous_len * 100`% der ursprünglichen Stationen. Mithilfe dieser Daten und des Bundesland Shapefiles lässt sich nun die Karte aller Bahnstationen erstellen (s. @fig-station-map)
...
In @fig-colored-station-map wird die Karte der Bahnstationen im bezug auf diese Kategorien eingefärbt. Der Transparenzeffekt soll dazu dienen die dominanten Farben in Clustern ersichtlich zu machen.
...
Insbesonderem Stuttgart fällt als am Stadt mit den höchst Kategorisierten Bahnstationen auf (s. @fig-colored-station-map).
Eine potentielle Ursache wird bei Betrachtung der Aufgabenträger ersichtlich (s. @fig-colored-station-map-responsible). Ähnlich wie in @fig-colored-station-map wird hierbei die Karte eingefärbt, da es zu keinen Überlappungen kommt kann hier auf den Transparenzwert verzichtet werden.
...
Im Gegensatz zu Bundesländern wie Bayern, Thüringen oder Sachsen-Anhalt hat Baden Württemberg einen seperaten Aufgabenträger für seine Hauptstadt.
Aufgabenträger finanzieren Bahninfrastruktur, Stationen und Bahnlinien.
Somit beinflussen Aufgabenträger auch die Leistungen eines Bahnhofs, die die dessen Kategorie bestimmen.
...
München, Berlin und Hamburg haben verhältnismäßig den höchten Anteil an Bahnhöfen der Kategorie 4, während Düsseldorf, Nürnberg und Dresden die höchsten Anteile an Bahnhöfen der Kategorie 5 besitzen.
Die in @fig-colored-station-map sichtbaren Schwerpunkte lassen sich daher bestätigen.
Bezüglich der Fragestellung lässt sich aussagen, das die Kategorien der Bahnhöfe zwischen Städen ungleichmäßig verteilt sind. 
...
Hierzu wird dem Dataframe `german_states` die Anzahl der Bahnhöfe in diesem Bundesland hinzugefügt. Darauf werden 3 Karten zur Befölkerungsdichte, Bahnhofsdichte und Anzahl der Bahnhöfe pro Person erstellt. Diese werden nebeneinander angezeigt um die verschiedenen Daten gut vergleichen zu können.
...
Um den im Bundesländervergleich markanten Unterschied genauer zu analysieren wird nun die Granularität mithilfe von Postleitzahlen(PLZ) erhöht. Dies wird in zwei schritten erledigt. Zuerst werden Gebiete mithilfe der ersten beiden Ziffern in der Postleitzahl (Postleitregionen) gruppiert. Danach wird jede PLZ einzeln betrachtet.

Zunächst müssen die Daten wieder angepasst werden. Hierzu werden die Postleitzahlen nach den ersten beiden Ziffern gruppiert und die Summe aller Einwohner sowie aller Bahnhöfe in diesem Gebiet hinzugefügt. Dieser Prozess ist sehr rechenintensiv. Darauf wird mit `st_area` die Fläche jedes Gebietes aus den Geometriedaten berechnet. Zusätzlich werden hier outlier herrausgefiltert. In diesem Fall sind das PLZ welche unter 30 Einwohner haben. Diese PLZ beziehen sich auf Spezialfälle wie Flughäfen mit mehreren Bahnhöfen und kaum Einwohnern. Im Anschluss werden wieder drei Karten zur Bevölkerungsdichte, Bahnhofsdichte und Anzahl der Bahnhöfe pro Person erstellt.
...
Mit diesen drei Karten kann man die vorhergehenden Vermutungen aus @sec-map-bundeslander spezifizieren. Es sind nun in der dritten Karte drei Regionen (Ost, Nordwest und Süd) zu erkennen, welche eine unterschiedliche Anzahl an Bahnhöfen pro Person haben.

#### Aufteilung in einzelne Postleitzahlen {#sec-plz-map}

Im letzen Schritt der Analyse mit Karten werden nun die Bereiche in die einzelnen Postleitzahlregionen unterteilt. Hier ergibt sich nun auch die Besonderheit, dass es für einige PLZ keinen Bahnhof gibt. Dennoch muss hier keine weitere Datenmanipulation erfolgen und es kann mit den verarbeiteten Daten aus den vorherigen Schritten gearbeitet werden.
...
Mit dieser feien Granularität sieht man nun, dass die Bahnhofsdichte stark mit der Bevölkerungsdichte zusammenhängt. Jedoch ist das Verhältniss Einwohner pro Bahnhof in Deutschland recht gleichmäßig verteilt. Es gibt keinen großen Unterschied zwischen Land- und Stadtregionen. Dies gilt zumindest an den Postleitzahlen, an welchen es mindestens einen Bahnhof gibt. Erstaunlicherweise hat die Karte der Bahnhofsdichte (und somit auch die Dritte) großflächige Lücken (graue Stellen) an welchen es gar keinen Bahnhof gibt. Dennoch kann man feststellen das die Lücken im Osten des Landes deutlich kleiner sind und es somit in fast jeder PLZ mindestens einen Bahnhof gibt. Dies kann jedoch auch damit zusammenhängen, dass die PLZ-Regionen sehr unterschiedlich groß sind. Mehr dazu in @sec-grid .
...
Damit der Graph aufgrund der vielen Postleitzahlen mit "0"-Werten nicht verfälscht wird, muss wie in den bisherigen Graphiken eine logarithmische Skalierung gewählt werden.
...
```
Erkennbar ist das wie erwartet höhere Einwohnerdichten mit höheren Bahnhofsdichten verwand sind.
Die bei der Postleitzahlgraphik geäußerte Vermutung lässt sich daher bestätigen.

### Versuch auf einem Grid {#sec-grid}

In @fig-plz-tripple-plot ist in Ostdeutschland eine großere Flächenabdeckung der Postleitzahlen mit Bahnhöfen zu sehen als im Westen.
Ob dies einer real gleichmäßigeren Flächenabdeckung zu Grunde liegt oder einfach an den größeren Postleitzahlflächen in Ostdeutschland liegt soll mit einer gleichmäßig granularer Karte geprüft werden.
...
Außnahmen bilden hier wie zuvor Populationszentren wie Frankfurt, Berlin oder München.

### Fun Fact: Prozentsatz an Bahnhöfen mit "Bahnhof" in der Adresse

Abseits der Fragestellung, ist bei der Analyse der Adressen aufgefallen, dass die aller meisten Straßennahnmen "Bahnhof" im Namen tragen (bspw. `Bahnhofstraße`, `Bahhofallee`).
Um zu überprüfen ob sich diese Beobachtung im gesammten Datensatz wiederspiegelt wird folgendes getestet:
...
Und es stellt sich herraus, dass tatsächlich `r address.percentage`% aller Bahnhöfe "Bahnhof" in ihrer Adresse stehen haben!

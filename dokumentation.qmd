...
```{r}
germany_matrix <- matrix(rep(0, 200*200), nrow = 200, ncol = 200)

# Dim: 5.5-15.5 x 46-56

# we need it to be sequential
for(x in 1:nrow(stations_geo_loc)) {
  lon <- round((stations_geo_loc[x, "lon"] - 5.5) * 20)[1, 1]
  lat <- round((stations_geo_loc[x, "lat"] - 46) * 20)[1, 1]
...
t.order
t.order[1:12,]$Var1
filter(bahnhof, Ort %in% t.order[1:12,]$Var1)

str(bahnhof)


top10 = filter(bahnhof, Ort %in% t.order[1:12,]$Var1)

top10 <- top10 %>% left_join(t.order, by = c("Ort" = "Var1")) 
...
ggplot(data=top10) +
  geom_bar(
    aes(
      x=reorder(Ort, Freq, decreasing = T), fill=`Kat. Vst`)
    ,
    #stat="identity"
  ) + 
  theme(
